---
title: "Resume"
description: "Ravi Baskaran's resume."
---

# Ravi Baskaran

Chief Product & Technology Officer

Chief Product & Technology Officer at Justo. 20+ years building and commercializing B2B products across SaaS, banking, ecommerce, real estate, accounting and telecom.

I have spent over two decades architecting B2B platforms, securing 14 patents in core infrastructure and driving major commercial outcomes like $40M+ in incremental revenue and a 2M+ user cloud ecosystem. Most recently, I anchored an IPO by transforming manual enterprise operations into measurable, AI-driven agentic workflows.

## Contact

- Email: [brkumar@gmail.com](mailto:brkumar@gmail.com)
- LinkedIn: [linkedin.com/in/brkumar](https://linkedin.com/in/brkumar)
- X: [@ravibaskaran](https://x.com/ravibaskaran)

## Summary

Hands-on Chief Product & Technology Officer at Justo, where technology anchored the largest pie in the IPO use of proceeds. Comfortable taking a product from 0 to 1 inside a startup and scaling it inside a Fortune 500. Range covers product strategy, architecture, engineering execution, GTM, client success, P&L, and CxO engagement.

## Experience

### Justo

**Chief Product & Technology Officer**  
_Jan 2025 to present_

- Reports to the CEO and leads product, technology, and AI.
- Moved the business from manual operating processes into an enterprise built around AI.
- Built CRM, ERP, and workflow systems from the ground up under the internal product name Manthan.
- Embedded GenAI and agentic workflows across calling, sourcing, closing, collections, and site operations.
- Ran build-versus-buy decisions on major investments and won investor backing for the full tech and AI program.
- Internal evidence showed roughly 30% reduction in CAC and a similar lift in prospect-to-customer conversion.

### Algonomy

**Director of Products**  
_Feb 2022 to Jan 2025_

- Led product strategy and commercialization for Find & Discover in retail personalization.
- Hired across the stack from senior software engineer to VP Engineering and Director of Products.
- Integrated LLMs, vector search, and wisdom-of-crowds signals into personalization.
- Delivered real-time B2B search that balanced customer experience and commercial goals.
- Grew Find & Discover into a $2M ARR business and positioned it as the #2 credible player in personalized retail search.

### Tally Solutions

**Product Director**  
_Nov 2017 to Oct 2020_

- Led product strategy and value creation for cloud, GST, and banking services on one of India's most widely used SME accounting platforms.
- Created and launched Tally GST.
- Built the first GST and banking services on Tally Cloud.
- Led banking reconciliation and payment integrations with DBS, HDFC, Kotak, and others.
- Contributed to the TallyPrime launch and designed cloud architecture for low-cost operations.
- Scaled cloud, GST, and banking services to a 2M+ SME ecosystem.

### AMD

**Senior Manager and Director**  
_Aug 2011 to Sep 2014_

- Drove laptop business growth at AMD India by rethinking how benchmarks shape the product roadmap.
- Owned market requirements, roadmap, architecture, and engineering execution.
- Built automated systems for CPU and GPU benchmark collection and analysis.
- Won internal backing to plan the roadmap from analytics rather than executive intuition.
- Drove $40M in incremental laptop business revenue.

### Altisource

**Product & Programs Head**  
_2014 to 2017_

- Led a B2B marketplace and mortgage/loan origination platform spanning valuation, home inspection, and insurance.
- Managed a team of 60.
- Delivered $2M+ in revenue in two years.

### Nokia

**Principal Product Manager**  
_2005 to 2011_

- Led maps, location, calendar, and contacts across the Nokia smartphone line.
- Worked on mobile platform products during the smartphone era.

### The X Future

**CPO and Technology Head**  
_2020 to 2021_

- Built a B2B platform for corporates to scout and buy startup innovation.
- Led product and technology from concept through execution.

### Bell Labs and Verisign

**Engineering**  
_2001 to 2005_

- Worked on telecom HLR and VLR switches.
- Built the Datablitz in-memory database.
- Contributed to identity, DNS, and mobile data infrastructure.

## Patents & Publications

- 14 granted patents across telecom and mobile infrastructure.
- Mobile, Location, and Context, Nokia, 2005 to 2011.
- Identity, DNS, and Mobile Data, Verisign, 2003 to 2005.
- Telecom Switching and In-Memory Database, Lucent and Bell Labs, 2001 to 2003.
- AI in B2B Real Estate, 2025.
- Availability, Reliability and Security of Cloud Data, 2016.
- Semantic Location Prediction Using Context and Positioning, 2015.
- Economics of the Cloud Data Controller, 2013.

## Core Skills

- AI commercialization
- Business transformation
- 0 to 1 product build and scale
- Product and platform leadership
- Commercial and CxO engagement

## Education

- Bachelor of Engineering, Electronics and Communications, National Institute of Technology, Nagpur, 1996
- Executive development: PG Diploma in Managing Software Programs, HSE Finland; Advanced Negotiations and Managing People in Software Projects, IIM Bangalore; People, Process and Management, Stanford; Situational and Tactical Business Leadership, Ken Blanchard; Technology Strategy for R&D, Dr. Jay Paap, Caltech.
